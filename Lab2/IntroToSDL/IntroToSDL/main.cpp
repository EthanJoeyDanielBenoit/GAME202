...
int main(int argc, char *argv[])
{
	...
	srand(time(NULL));
	for (i = 0; i < NUM_SPRITES; ++i) {
		positions[i].x = rand() % (WINDOW_WIDTH - sprite_w); // rand() % between 0 to 799 (random positions for all sprites)
		positions[i].y = rand() % (WINDOW_HEIGHT - sprite_h); // rand() % between 0 to 599 (random positions for all sprites)
		positions[i].w = sprite_w;
		positions[i].h = sprite_h;
		velocities[i].x = 0;
		velocities[i].y = 0;
		while (!velocities[i].x && !velocities[i].y) {
			velocities[i].x = (rand() % (MAX_SPEED * 2 + 1)) - MAX_SPEED; // rand % 3 between -1 to 2
			velocities[i].y = (rand() % (MAX_SPEED * 2 + 1)) - MAX_SPEED;
		}
	}

	// sets the velocities for two sprites to only move straight in the horizontal and vertical destination 
	velocities[0].x = 1;
	velocities[0].y = 0;
	...
	while (!done) { // main game loop
		// Check for events
		while (SDL_PollEvent(&event)) { // main event loop
			if (event.type == SDL_QUIT || event.type == SDL_KEYDOWN) {
				done = 1;
			}
		}
		// Going to the right (possitive)
		if (velocities[0].x > 0) {
			velocities[0].x += 1;
		}
